...
4. **Generate the verification checklist — machine-verify first, persist second, present third**: based on the task's title + description and the `commitFrom..HEAD` diff, derive the acceptance checks; when there is a spec, cross-reference its design intent. Then split them:
   - **Machine-verifiable** (a command, test, validator, grep, or reading a file can decide pass/fail): the main thread **runs them itself** and records command + result. Never hand these to the user — asking a person to run a grep or open a file to read a line is wasted effort.
   - **Human-only** (visual appearance, interaction feel, subjective judgement, systems the AI can't reach): written as **actionable** steps (which page/element to open and click, what counts as passing).
   - The presentation leads with the machine results as a compact evidence block ("N checks run, all passed: …"), followed only by the human-only steps. If there are none, say so and ask for a single OK.
   - **With a spec**: first append a verification-record section to the end of the spec (heading in the spec's own language; machine-verified items already checked off with command + pass date, human-only items in unverified state `- [ ]`), then present it to the user, in the user's conversation language, along with the spec path. Do not ask the user to start verifying until the spec is persisted to disk.
   - **Without a spec**: present in conversation only, in the user's conversation language — the evidence block first, then any human-only steps in the format "Please verify: ① … ② …"
5. **Actual user verification** — wait for the user to run through the human-only steps (or, when there are none, to acknowledge the machine results) and report back; never decide pass/fail on the user's behalf. **Implementing and marking done in the same turn is prohibited**: the done write may only happen after the user has replied confirming verification — an uninterrupted implement → commit → done run is exactly the violation this step exists to stop
