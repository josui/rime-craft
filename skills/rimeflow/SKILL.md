...
├── dashboard-server.mjs ← Dashboard server（由 assets/ 复制）
└── anchors/        ← session 记录（自动生成，gitignore）
```

dashboard-server.mjs 从 `assets/dashboard-server.mjs` 复制，需要 Node.js 18+。
...
| 发现改善点 / 新想法 | 用 `/add-backlog` 添加到 tasks.json（status: todo） |
...
| rimeflow 插件更新后 | 对比 `.rime/dashboard-server.mjs` 头部版本号，低于插件版本则覆盖；无法解析则强制覆盖并警告 |
...
- 编号全局唯一，不回收不复用
- 用 `/add-backlog` 添加新 item 时自动分配编号
